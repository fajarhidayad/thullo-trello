import { AppstoreOutlined, CaretDownFilled } from '@ant-design/icons';
import {
  Avatar,
  Button,
  Divider,
...
export default function Navbar() {
  ...
  const match = matchPath({ path: '/boards/:id' }, location.pathname);

  return (
    <Header className="sticky top-0 z-10 w-full shadow bg-white py-4">
      <Flex align="center">
        <Link to={'/'} className="font-medium text-black mr-[94px]">
          <Logo />
        </Link>
        {match && (
          <Flex align="center">
            <Text className="font-medium text-lg">Title Board</Text>
            <Divider type="vertical" className="h-[35px] mx-6" />
            <Button
              type="default"
              onClick={() => navigate('/boards')}
              className="bg-gray-100 text-gray-600 text-xs font-medium flex items-center"
            >
              <AppstoreOutlined className="text-lg" /> All board
            </Button>
          </Flex>
        )}
        <Flex className="flex-shrink-0 space-x-10 ml-auto" align="center">
          <Search
            placeholder="Keyword..."
            allowClear
            enterButton="Search"
            size="middle"
            className="shadow-md w-[338px]"
          />
          <Dropdown
            menu={{
              items: menuItems,
            ...
            trigger={['click']}
            className="flex-shrink-0 font-bold text-xs text-black"
          >
            <a onClick={(e) => e.preventDefault()}>
              <Space>
                <Avatar className="font-normal">U</Avatar> User
                <CaretDownFilled />
              </Space>
            </a>
          </Dropdown>
